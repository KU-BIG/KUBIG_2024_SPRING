...
import sys
sys.path.append('/content/drive/MyDrive/KUBIG/CV 분반/방학 프로젝트/open_flamingo/open_flamingo/eval')
...
# Trial arguments
# shot default = [0,4,8,16,32]
parser.add_argument("--shots", nargs="+", default=[0,2,4], type=int)
parser.add_argument(
    "--num_trials",
    type=int,
    default=1,
...
def main():
    ...
    b = [i+1 for i in range(0, len(leftovers)-1, 2)]
    a = [leftovers[i] for i in range(0, len(leftovers), 2)]
    print(a)
    print(f"open_flamingo.eval.models.{args.model}")
    # module = importlib.import_module(f"open_flamingo.eval.models.{args.model}")

    model_args = {
        leftovers[i].lstrip("-"): leftovers[i+1] for i in range(0, len(leftovers)-1, 2)
    }
    print(model_args)
    eval_model = EvalModel(model_args) #module.EvalModel

    # # # set up distributed evaluation
    # args.local_rank = 0
    args.rank = 0
    args.world_size = 1 #world_info_from_env()
...
def evaluate_captioning(
    ...
    if args.rices:
        ...
        query_set = utils.get_query_set(test_dataset, args.query_set_size)
...
def evaluate_vqa(
    ...
    with open(f"{dataset_name}results_{random_uuid}.json", "w") as f:
        f.write(json.dumps(all_predictions, indent=2))
