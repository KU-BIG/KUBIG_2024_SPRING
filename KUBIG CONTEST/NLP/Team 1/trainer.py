...
def train(config, model, optimizer, tokenizer, train_dataloader, device, train_sampler=None):
    ...
    for epoch in range(config['EPOCHS']):
        ...
        if config['ddp']:
            if (epoch+1) % 5 == 0:
                model.module.save_pretrained(f"/home/thdtjd06/checkpoints/kogpt2_0221_epoch{epoch+1}")
                tokenizer.save_pretrained(f"/home/thdtjd06/checkpoints/kogpt2_0221_epoch{epoch+1}")
        else:
            if (epoch+1) % 5 == 0:
                model.save_pretrained(f"/home/thdtjd06/checkpoints/kogpt2_0221_epoch{epoch+1}")
                tokenizer.save_pretrained(f"/home/thdtjd06/checkpoints/kogpt2_0221_epoch{epoch+1}")
